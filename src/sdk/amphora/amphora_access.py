...
class AmphoraAccess(Base):
    ...
    def create_organisation_rule(self, organisation_id: str, allow_or_deny: str = 'Deny', priority: int = 100) -> api.OrganisationAccessRule:
        ...
        """
        rule = api.OrganisationAccessRule(organisation_id=organisation_id, allow_or_deny=allow_or_deny, priority=priority)
        return self.amphoraeApi.amphorae_access_controls_create_for_organisation(id = self._id, organisation_access_rule=rule)

    def create_user_rule(self, username: str, allow_or_deny: str = 'Deny', priority: int = 100) -> api.UserAccessRule:
        """
        Creates a rule by Organisation on this Amphora
        params:
        ...
        """
        rule = api.UserAccessRule(username=username, allow_or_deny=allow_or_deny, priority=priority)
        return self.amphoraeApi.amphorae_access_controls_create_for_user(id = self._id, user_access_rule=rule)
    ...
    def delete_rule(self, rule_id: str):
        ...
        returns:
            amphora_api_client.UserAccessRule      
        """
        return self.amphoraeApi.amphorae_access_controls_delete(id = self._id, rule_id= rule_id)